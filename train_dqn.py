...
while True:
    ...
    if reward is not None:  # end of epsiode
        ...
        print(f"frame {frame_idx}, game {game_idx}, replay memory size= {len(agent.replay_memory)}")
        # get mean reward every 100 games
        if game_idx % 100 == 0:
            mean_reward = np.mean(total_rewards[-100:])
            print(f"mean reward over games {len(total_rewards)-99}:{len(total_rewards)} = {mean_reward:.3f}")
